...
def imwrite(cube_frame_path, pyra_frame_path, json_path, write_path, frame_idx, fps, duration_unit, salient_patch_size, tar_num, output):#newly added: frame_idx, fps, duration_unit,

    if not os.path.exists(write_path):
        #print(json_path)
        cube_frame = cv2.imread(cube_frame_path)
        pyra_frame = cv2.imread(pyra_frame_path)
    ...
    for duration_unit in duration_units:
        ...
        for participant_name in os.listdir(csv_base_path):
            ...
            for frame_name, json_path in zip(sorted_frame_name_list, user_json_path_array):
                ...
                cube_frame_path = os.path.join(cube_duration_path, frame_name)
                pyra_frame_path = os.path.join(pyra_duration_path, frame_name)

                cube_frame_input_list.append(cube_frame_path)
                pyra_frame_input_list.append(pyra_frame_path)
